package engine.saga.action
{
   ...
   public class Action_BattleWaitTurnAbility extends Action
   {
       
      
      private var waitForIt:IBattleEntity;
      
      public function Action_BattleWaitTurnAbility(param1:ActionDef, param2:Saga)
      {
         super(param1,param2);
      }
      ...
      override protected function handleStarted() : void
      {
         ...
         if(def.id)
         {
            ...
            if(Boolean(_loc3_) && Boolean(_loc3_.fsm))
            {
               ...
               if(this.waitForIt)
               {
                  _loc3_.fsm.addEventListener(BattleFsmEvent.TURN_ABILITY,this.battleFsmAbilityHandler);
                  return;
               }
      ...
      private function battleFsmAbilityHandler(param1:BattleFsmEvent) : void
      {
         var _loc2_:BattleBoard = saga.getBattleBoard();
         var _loc3_:BattleFsm = !!_loc2_ ? _loc2_.fsm as BattleFsm : null;
         if(_loc3_)
         {
            if(!_loc3_.turn || !_loc3_.ability || _loc3_.ability.def.id != def.param)
            {
               return;
            }
         }
         param1.target.removeEventListener(BattleFsmEvent.INTERACT,this.battleFsmAbilityHandler);
         end();
      }
   }
}
